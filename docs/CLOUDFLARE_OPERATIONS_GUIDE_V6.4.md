# Hariyo Mart Nepal v6.4 — Cloudflare Operations Guide
...
The production finisher creates the first owner through a secure prompt when none exists:

```bash
npm run finish:cloudflare
```

It asks for the password twice with terminal echo disabled, verifies an actual sign-in, and rotates
the one-time bootstrap secret automatically. To run just this step against an already-deployed
Worker, use `npm run bootstrap:admin`.

Defaults:

- email: `greenmandux@gmail.com`
- role: platform admin/owner
- password: requested twice, 14+ characters with uppercase, lowercase, number and symbol
...
bootstrap secret for defence in depth. The API also refuses every later bootstrap attempt once any
admin exists:
...
The deployed admin can change the owner password under **Admin → Settings**. The endpoint verifies
the current password, hashes the replacement with bcrypt and revokes all sessions so every device
must sign in again.

## 5. D1 migrations, seed and recovery

Migrations are ordered SQL files in `apps/web/migrations`. The generated seed is idempotent and adds
98 products plus realistic orders, fulfilments, inventory events, promotions, reviews, support,
CMS content and notifications. Seed users use `.invalid` addresses and disabled password hashes;
they populate operational dashboards but can never authenticate.
...
consistent point for each object ID. Checkout requests are keyed by a stable hash of their sorted
product set, so carts touching the same inventory serialize while unrelated catalog orders remain
parallel. Keep object state small, use SQLite-backed classes, and do not route every unrelated
request through one global object. Inventory and final order records still belong in D1.
