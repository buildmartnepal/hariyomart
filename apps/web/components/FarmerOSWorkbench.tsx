...
type ActionSectionProps = {
  ...
  error: string;
  notice: string;
  onSubmit: (payload: JsonRecord) => void | Promise<void>;
  refresh: () => void;
};
...
function FarmPlanning({ data,busy,error,notice,onSubmit,refresh }: ActionSectionProps) {
...
function Profitability({data,busy,error,notice,onSubmit,refresh}: ActionSectionProps){
...
function Recommendation({item}:{item:JsonRecord}){const priority=text(item.priority||'medium');const actionHref=text(item.actionHref);return <div className={`farmer-os-recommendation priority-${priority}`}><div className="farmer-os-rec-icon">{priority==='critical'||priority==='high'?<AlertTriangle size={17}/>:<Sparkles size={17}/>}</div><div><span>{text(item.type).toUpperCase()} · {priority.toUpperCase()}</span><strong>{text(item.title)}</strong><p>{text(item.message)}</p>{actionHref?<Link href={actionHref}>{text(item.actionLabel||'Open')}</Link>:null}</div></div>}
